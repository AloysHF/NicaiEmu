...
This document describes how to publish a new release of NicaiEmu.
...
| `crates/nicaiemu-libretro/nicaiemu_libretro.info` | `display_version` | `"0.1.0"` |
...
sed -i 's/^display_version = "0.1.0"/display_version = "0.2.0"/' \
  crates/nicaiemu-libretro/nicaiemu_libretro.info
```

### 2. Commit the version bump

```bash
git add Cargo.toml Cargo.lock crates/nicaiemu-libretro/nicaiemu_libretro.info
...
2. **Builds libretro cores** for the same platforms, renaming the cdylib to
   `nicaiemu_libretro.<ext>` and bundling `nicaiemu_libretro.info`
...
1. Go to [Releases](https://github.com/jiangxincode/NicaiEmu/releases)
...
4. Verify all expected artifacts are attached:
   - `nicaiemu-linux-x86_64.tar.gz`
   - `nicaiemu-macos-x86_64.tar.gz`
   - `nicaiemu-macos-aarch64.tar.gz`
   - `nicaiemu-windows-x86_64.zip`
...
(`dist/info/nicaiemu_libretro.info`), not from this repo. If the `.info` file
...
2. Copy `crates/nicaiemu-libretro/nicaiemu_libretro.info` from this repo
   to `dist/info/nicaiemu_libretro.info` in the fork
3. Submit a PR to `libretro/libretro-super` — reference the NicaiEmu release
...
- Check the [Actions](https://github.com/jiangxincode/NicaiEmu/actions) tab
  for the failed run
...
[Releases](https://github.com/jiangxincode/NicaiEmu/releases) page before
...
- Ensure both files have the same version string:
  `Cargo.toml` and `nicaiemu_libretro.info`
